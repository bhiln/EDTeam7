...
void i2c_int_handler() {
        ...
        switch (ic_ptr->status) {
            ...
            case I2C_STARTED:
            {
                ...
                if (SSPSTATbits.P == 1) {
                    ...
                    ic_ptr->event_count++;
                    if (SSPSTATbits.D_A == 0) {
                        if (SSPSTATbits.R_W == 0) { // write to slave
                            ic_ptr->status = I2C_RCV_DATA;
                        } else { // read the slave
                            ic_ptr->status = I2C_SLAVE_SEND;
                            msg_to_send = 1;
    ...
    if (msg_ready) {
        ...
        ToMainHigh_sendmsg(ic_ptr->buflen, MSGT_I2C_SLAVE_RECV_COMPLETE, (void *) ic_ptr->buffer);
        ic_ptr->buflen = 0;
...
//        if (send == 1) {
//            if (ic_ptr->outbuffer[0] == 0x35) {
//                ic_ptr->outbuffer[0] = 0x35;
//            } else {
//                ic_ptr->outbuffer[0] = 0x0;
//            }
//            start_i2c_slave_reply(1, ic_ptr->outbuffer);
//        }
//        else {
//            ic_ptr->outbuffer[0] = 0x0;
//            start_i2c_slave_reply(1, ic_ptr->outbuffer);
...
void i2c_configure_slave(unsigned char addr) {
    ...
    SSP1CON1 = 0x0;
    SSP1CON2 = 0x0;
    SSP1CON1bits.SSPM |= 0x0E; // enable Slave 7-bit w/ start/stop interrupts

    // ensure the two lines are set for input (we are a slave)
    TRISCbits.TRISC3 = 1; // RC3 = SCL1
    TRISCbits.TRISC4 = 1; // RC4 = SDA1
