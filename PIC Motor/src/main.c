...
//    i2c_master_send(0x9A, 6, imsg); // send test msg to motor pic
//    i2c_master_send(0x9A, 6, jmsg); // send test msg to motor pic
    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                case MSGT_I2C_SENSOR_RQST:
                {
//                    FromMainHigh_recvmsg(MAXI2CBUF, I2C_SENSOR_SEND, ); // send sensor data from FromMainHigh_MQ
                    break;
                };
                case MSGT_I2C_MOTOR_RQST:
                {
//                    FromMainHigh_recvmsg(MAXI2CBUF, I2C_MOTOR_SEND, ); // send motor data from FromMainHigh_MQ
                    break;
                };
                case MSGT_I2C_RQST:
                {
...
void main(void) {
    ...
    unsigned char jmsg[4];
    unsigned char kmsg[4];
    imsg[0] = 0x11;
    imsg[1] = 0x22;
    imsg[2] = 0x33;
    imsg[3] = 0x44;
    ...
    //i2c_master_send(4, jmsg, 0x7D);
    //i2c_master_send(4, kmsg, 0x5B);
    while (1) {
        // Call a routine that blocks until either on the incoming
        // messages queues has a message (this may put the processor into
        // an idle mode)
...
//                }
            ...
            switch (msgtype) {
                ...
                case MSGT_I2C_SENSOR_DATA: {

                    break;
                };
                case MSGT_I2C_QE_DATA: {
...
//                            }
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                case MSGT_TIMER1: {
                    DEBUG_ON(TIMER1_MSG_RCV);
                    timer1_lthread(&t1thread_data, msgtype, length, msgbuffer);
                    DEBUG_OFF(TIMER1_MSG_RCV);
                    break;
                };
                case MSGT_OVERRUN: {
...
//                    uart_lthread(&uthread_data, msgtype, length, &val);
                ...
                case MSGT_SENSOR_DATA_RQST: {

                    break;
                };
                case MSGT_SENSOR_DATA: {
                    // Send sensor data to UART
                    SensorData_sendmsg(MSGLEN, MSGT_I2C_RQST, (void *) msgbuffer);
                    break;
                };
                default: {
                    // Your code should handle this error
...
>>>>>>> FETCH_HEAD:PIC Motor/src/src/main.c
}